...
def main():
    ...
    while True:
        ...
        if OBJECT_NAME in gazebo_model_states.name:
            ...
            #近づく
            theta = math.atan(object_position.y / object_position.x)
            target_pose.position.x = object_position.x
            target_pose.position.y = object_position.y - 0.08
            ...
            #左にふる
            target_pose.position.x = 0.3*math.cos(math.pi/6)
            target_pose.position.y = 0.3*math.sin(math.pi/6)
            q = quaternion_from_euler(-math.pi/6, math.pi/2, 0.0)
            target_pose.orientation.x = q[0]
            target_pose.orientation.y = q[1]
            target_pose.orientation.z = q[2]
            target_pose.orientation.w = q[3]
            arm.set_pose_target(target_pose)
            if arm.go() is False:
                print "Failed to turn left an object."
                continue
            rospy.sleep(0.1)
            ...
            arm.set_pose_target(target_pose)
            if arm.go() is False:
                print "Failed to center an object."
                continue
            rospy.sleep(1.0)
            ...
            arm.set_pose_target(target_pose)
            if arm.go() is False:
                print "Failed to placean object."
                continue
            rospy.sleep(1.0)
            gripper_goal.command.position = GRIPPER_OPEN
            gripper.send_goal(gripper_goal)
            ...
            arm.set_named_target("home")
            if arm.go() is False:
                print "False to go back to home pose."
                continue
            rospy.sleep(1.0)
...
if __name__ == '__main__':
    rospy.init_node("pick_and_move_example")
