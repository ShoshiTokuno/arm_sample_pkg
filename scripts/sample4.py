...
def main():
    ...
    while True:
        ...
        if OBJECT_NAME in gazebo_model_states.name:
            ...
            #左にふる
            target_pose.position.x = 0.4*math.cos(math.pi/4)
            target_pose.position.y = 0.4*math.sin(math.pi/4)
            q = quaternion_from_euler(-math.pi/2, 0.0, -math.pi/4)
            target_pose.orientation.x = q[0]
            target_pose.orientation.y = q[1]
            target_pose.orientation.z = q[2]
            target_pose.orientation.w = q[3]
            ...
            target_pose.position.x = 0.4*math.cos(-math.pi/4)
            target_pose.position.y = 0.4*math.sin(-math.pi/4)
            q = quaternion_from_euler(-math.pi/2, 0.0, -3*math.pi/4)
            target_pose.orientation.x = q[0]
            target_pose.orientation.y = q[1]
            target_pose.orientation.z = q[2]
            target_pose.orientation.w = q[3]
